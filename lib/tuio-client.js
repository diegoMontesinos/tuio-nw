...
module.exports = function (Tuio) {
  Tuio.Client = Tuio.Model.extend({
    ...
    initialize: function (params) {
      ...
      this.oscParser = require('./osc-parser.js');
      ...
      Tuio._.bindAll(this, 'onListening', 'acceptBundle', 'onClose');
    },
    ...
    onListening: function () {
      ...
      this.socket.on('message', function (msg) {
        var oscBundle = that.oscParser.decode(msg);
        that.acceptBundle(oscBundle);
      });
      this.connected = true;
      this.trigger('listening');

      var address = this.socket.address();
      console.log('TuioClient listening on: ' + address.address + ':' + address.port);
    },
    ...
    getTuioObjects: function () {
      return Tuio._.clone(this.objectList);
    },

    getTuioCursors: function () {
      return Tuio._.clone(this.cursorList);
    },
    ...
    objectSet: function (args) {
      ...
      if (!Tuio._.has(this.objectList, sid)) {
    ...
    objectAlive: function (args) {
      ...
      this.aliveObjectList = Tuio._.difference(this.aliveObjectList, this.newObjectList);
    ...
    cursorSet: function (args) {
      ...
      if (!Tuio._.has(this.cursorList, sid)) {
    ...
    cursorAlive: function (args) {
      ...
      this.aliveCursorList = Tuio._.difference(this.aliveCursorList, this.newCursorList);
    ...
    cursorRemoved: function (tcur) {
      ...
      if (removeCursor.getCursorId() === this.maxCursorId) {
        this.maxCursorId = -1;
        if (Tuio._.size(this.cursorList) > 0) {
          var maxCursor = Tuio._.max(this.cursorList, function (cur) {
          ...
          this.freeCursorList = Tuio._.without(this.freeCursorList, function (cur) {
    ...
    cursorAdded: function (tcur) {
      var cid = Tuio._.size(this.cursorList),
        testCursor = null;
